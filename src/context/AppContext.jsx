...
// Axios Configuration: Cross-origin cookies ke liye zaroori
axios.defaults.withCredentials = true;
axios.defaults.baseURL =
  import.meta.env.VITE_BACKEND_URL || "https://server-kohl-nine-68.vercel.app";
...
export const AppContextProvider = ({ children }) => {
  ...
  const fetchUserData = async () => {
    try {
      // Backend se user ka session check karein
      const { data } = await axios.get("/api/user/is-auth");
      if (data.success) {
        setUser(data.user);
        const sellerStatus = data.user.isSeller || false;
      ...
      // Agar session expire ho gaya, to user state null rahega
      setUser(null);
  ...
  const addToCart = async (id) => {
    let updatedCart = { ...cartItems, [id]: (cartItems[id] || 0) + 1 };
  ...
  // Initial load par products aur user session fetch karein
  useEffect(() => {
    fetchProducts();
    fetchUserData();
  }, []);

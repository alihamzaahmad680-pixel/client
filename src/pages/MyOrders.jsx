...
const MyOrders = () => {
  ...
  const fetchMyOrders = async () => {
    try {
      ...
      console.log(error.message);
      toast.error(
        error.response?.data?.message || "Orders sync karne mein masla aya",
      );
    }
  };
          ...
          >
            ...
            {order.items.map((item, itemIndex) => (
              <div
                ...
                } border-gray-100 flex flex-col md:flex-row md:items-center justify-between py-4 md:gap-16 w-full`}
              >
                <div className="flex items-center mb-4 md:mb-0">
                  <div className="bg-primary/5 p-2 rounded-lg border border-gray-100">
                    <img
